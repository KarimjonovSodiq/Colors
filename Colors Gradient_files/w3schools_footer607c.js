...
(function () {
  ...
  for (i = 0; i < x.length; i++) {
    if (d.indexOf(x[i].href) >= 0) {
      ...
      break;
       ...
       if (x[i].href == d.substr(0, d.indexOf("../tags/index.html")) + "/tags/tag_" + c + ".asp") {
         x[i].className = "active";
       }
  ...
  for (i = 0; i < x.length; i++) {
    ...
    b = x[i].pathname;
    if ((x[i].parentNode.tagName == "LI" || x[i].parentNode.className.indexOf("w3-bar") > -1) && a.substr(0, a.indexOf("../index.html",1)) ==  b.substr(0, b.indexOf("../index.html",1))) {
      x[i].className += " active";
    }
  }
  if (window.addEventListener) { 
...
function gTra(el) {
  ...
  gtra.src = '../../translate.google.com/translate_a/elementa0d8.html?cb=googleTranslateElementInit';
...
function login_user(event) {
  ...
  xhttp.open("POST.html", "https://mypage.w3schools.com/mypage/login_user.php", true);
